...
const AnimatedTaskLabel = memo((props: Props) => {
  ...
  const strikethrougWidth = useSharedValue(0);
  const strikethroughAnimatedStyles = useAnimatedStyle(
    () => ({
      width: `${strikethrougWidth.value * 100}%`,
  ...
  useEffect(() => {
    ...
    if (strikethrough) {
      ...
      strikethrougWidth.value = withTiming(1, {duration: 400, easing});
      textColorProgress.value = withDelay(
        1000,
        withTiming(1, {duration: 400, easing}),
      );
    } else {
      strikethrougWidth.value = withTiming(0, {duration: 400, easing});
